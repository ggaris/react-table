		...
		return (
			<div className="flex justify-center">
				<div className="flex justify-center items-center gap-2 min-w-40">
					...
					{/* 百分比文字 */}
					<span
						className={`font-mono text-xs font-semibold tabular-nums ${getColor()} min-w-10 text-right`}
					>
						{formatted}
		...
		<div className="flex justify-center">
			<span className="flex justify-center gap-2">
				...
				<span className="relative flex justify-center">
					...
					<span
						className={`absolute w-1.5 h-1.5 rounded-full ${getIndicatorColor()} animate-ping opacity-75`}
					/>
				</span>

				{/* 百分比值 */}
				<span
					className={`font-mono text-sm font-semibold tabular-nums transition-colors duration-200 ${getColor()}`}
				>
					{formatted}
