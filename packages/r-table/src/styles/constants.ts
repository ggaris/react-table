...
// ========== 基础过渡动画 ==========
export const TRANSITION_BASE = "transition-all duration-150 motion-reduce:transition-none";
export const TRANSITION_COLORS = "transition-colors duration-150 motion-reduce:transition-none";
export const TRANSITION_FAST = "transition-all duration-100 motion-reduce:transition-none";
export const TRANSITION_SLOW = "transition-all duration-200 motion-reduce:transition-none";

// ========== 焦点环样式 ==========
export const FOCUS_RING = "focus:outline-none focus:ring-2 focus:ring-blue-500";
export const FOCUS_RING_OFFSET = "focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1";
...
// ========== 弹窗/下拉面板样式 ==========
export const DROPDOWN_PANEL = "absolute right-0 top-full mt-2 z-20 bg-white border border-gray-200 rounded-lg shadow-xl animate-in fade-in duration-150";
...
// ========== 徽章/标签样式 ==========
export const BADGE_BASE = "inline-flex items-center gap-1.5 text-xs font-medium rounded-md border";
...
// ========== 空状态样式 ==========
export const EMPTY_STATE_TEXT = "inline-flex items-center gap-1.5 text-slate-400 text-sm";
export const EMPTY_STATE_ICON = "w-3.5 h-3.5";

// ========== 错误状态样式 ==========
export const ERROR_BADGE = "inline-flex items-center gap-1.5 px-2 py-1 bg-red-50 text-red-600 text-xs rounded-md border border-red-200";

// ========== 表格相关样式 ==========
export const TABLE_HEADER_CELL = "text-xs font-semibold text-gray-800 uppercase tracking-wide border-b border-gray-200 bg-gray-50 relative";
...
export const TABLE_CONTAINER = "relative overflow-hidden border border-gray-200 rounded-lg shadow-sm bg-white";
...
// ========== 固定列边框样式 ==========
export const FIXED_COLUMN_BORDER_LEFT = "after:content-[''] after:absolute after:top-0 after:right-0 after:bottom-0 after:w-[1px] after:bg-gray-300 after:pointer-events-none after:shadow-[2px_0_4px_rgba(0,0,0,0.08)]";
export const FIXED_COLUMN_BORDER_RIGHT = "before:content-[''] before:absolute before:top-0 before:left-0 before:bottom-0 before:w-[1px] before:bg-gray-300 before:pointer-events-none before:shadow-[-2px_0_4px_rgba(0,0,0,0.08)]";
export const COLUMN_BORDER = "after:content-[''] after:absolute after:top-0 after:right-0 after:bottom-0 after:w-[1px] after:bg-gray-200 after:pointer-events-none";
 ...
 * 获取对齐样式类
 */
export function getAlignClass(align: "left" | "center" | "right" = "left"): string {
 ...
 * 获取密度内边距类
 */
export function getDensityPadding(density: "compact" | "default" | "comfortable" = "default"): string {
 ...
 * 获取密度表头内边距类
 */
export function getDensityHeaderPadding(density: "compact" | "default" | "comfortable" = "default"): string {
 ...
 */
export function getPercentageBgColor(percentage: number): { text: string; bg: string } {
	if (percentage >= 80) return { text: "text-emerald-600", bg: STATE_BG_COLORS.success.base };
	if (percentage >= 60) return { text: "text-blue-600", bg: STATE_BG_COLORS.info.base };
	if (percentage >= 40) return { text: "text-amber-600", bg: STATE_BG_COLORS.warning.base };
	if (percentage >= 20) return { text: "text-orange-600", bg: STATE_BG_COLORS.low.base };
 ...
 */
export function getProgressGradient(progress: number): { gradient: string; text: string } {
	if (progress >= 80) return { gradient: "from-emerald-500 to-emerald-400", text: "text-emerald-600" };
	if (progress >= 60) return { gradient: "from-blue-500 to-blue-400", text: "text-blue-600" };
	if (progress >= 40) return { gradient: "from-amber-500 to-amber-400", text: "text-amber-600" };
	if (progress >= 20) return { gradient: "from-orange-500 to-orange-400", text: "text-orange-600" };
